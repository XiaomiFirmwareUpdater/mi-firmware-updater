...
async def main(mode: str, links_file: Optional[Path] = None, roms_dir: Optional[Path] = None):
    ...
    if mode == 'auto':
        latest_roms = [
            i
            for i in session.query(latest_updates).filter(latest_updates.c.method == 'Recovery')
            if i.codename in get_current_devices()
        ]
        latest_files: set = {i.filename for i in latest_roms}
        latest_firmware_files: set = {
            '_'.join(i.filename.split('_')[2:]) for i in session.query(latest_firmware)
        }
        new_roms = [
            i for i in latest_roms if i.filename in latest_files.difference(latest_firmware_files)
        ]
    elif mode == 'manual':
        links = links_file.read_text().splitlines()
        new_roms = [MiuiRom(link) for link in links]
    elif mode == 'offline':
        roms = roms_dir.glob('*.zip')
